...
function Uninstall-ADTDeployment
{
	...
	try {
		$remainingApps = Get-ADTRegistryKey -Key $registryPath -ContinueOnError $true
		$appCount = if ($remainingApps -is [array]) { $remainingApps.Count } else { if ($remainingApps) { 1 } else { 0 } }
